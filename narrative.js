...
function setState(act,step,progress){
 ...
 act.style.setProperty('--path',(step/3).toFixed(4));
...
function render(){scheduled=false;for(const act of acts){const r=act.getBoundingClientRect();const p=enabled?clamp(-r.top/Math.max(1,act.offsetHeight-innerHeight)):1;const step=Math.min(3,Math.floor(p*4));setState(act,step,p)}}
...
function select(act,step){const progress=[.06,.37,.66,.94][step];setState(act,step,progress);if(enabled){const top=act.getBoundingClientRect().top+scrollY;scrollTo({top:top+progress*(act.offsetHeight-innerHeight),behavior:'instant'})}}
...
window.advanceNarrative=(act,direction)=>{if(!enabled||!act?.matches('[data-narrative]'))return false;const step=+(act.dataset.narrativeStep||0)+direction;if(step<0||step>3)return false;select(act,step);return true};
